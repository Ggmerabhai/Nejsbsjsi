...
bot = telebot.TeleBot('7285605333:AAGjRB13Ciiq5afuXIg8p0YAkIFFmcNyaNw')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown . Please Wait 30sec Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # @PrivateFileTg # Updated to accept target, time, and port
            ...
            response = "✅ Usage :- /attack <target> <port> <time>"  # @PrivateFileTg # Updated command syntax
    else:
